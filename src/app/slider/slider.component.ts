...
export class SliderComponent implements OnInit {
  slides:any[] = [{header:"Кофе как еда", subtitle:"В Эфиопии из сырых измельчённых кофейных ягод лепили съедобные шарики.<br><br><span>В одной кофейной ягоде обычно два зерна.</span>", description:"Абсолютное большинство кофеманов <span>(96%)</span> пьют кофе дома, половина из них <span>(54%)</span> продолжают пить кофе на работе, и лишь каждый пятый любитель просит чашку кофе в гостях.<br><br><span>3 миллиарда</span> чашек кофе выпивается в мире ежедневно."},
                  ...
                  {header:"Капсульные кофемашины", subtitle:"Инженер Эрик Фавр увёз из Италии секрет римского бариста, доработал его и запатентовал кофейную капсулу, которая прокалывается в процессе приготовления напитка. Сегодня эта технология доведена до совершенства: свежемолотый и свежеобжаренный кофе попадает в капсулу в вакуумной атмосфере — это один из этапов заботы о сохранении вкуса и аромата кофе, второй— алюминиевая капсула. На производстве она запечатывается, чтобы сохранить всю свежесть зерен до момента дегустации кофе. Именно это позволяет клиентам Nespresso наслаждаться идеальным напитком чашка за чашкой.", description:"Кофемашины Nespresso – любимый кофе одним касанием."}];
  idx:number = 0;
